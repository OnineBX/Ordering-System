...
public enum WorkDataCache {
	...
	private Map<Integer, StyledAdapterItem> foodMap 	= new ConcurrentHashMap<Integer, StyledAdapterItem>();	
	...
	public synchronized void lodingFoodData(Set<Integer> fssIdSet){
		...
		StyledAdapterItem foodItem;		
		FoodPriceEntity fpEnt;
		for(FoodEntity ent: foodList){
			fpEnt = foodManager.getDefaultPrice(ent.getId());
			String price = String.format("%s/%s", fpEnt.getPrice(),fpEnt.getUnit().getName());
			foodItem = new StyledAdapterItem(ent.getFoodSubSort().getId(), foodColor, String.format("%s\n%s", ent.getName(), price), ent.getIndex());
	 ...
	 * @return 菜品数据
	 */
	public synchronized Map<Integer, StyledAdapterItem> getFoodData(){
		return foodMap;
	}			
